...
export default function Layout({ children }) {
  const { enlarged, toggle } = useTextSize();
    ...
    <div className="min-h-screen bg-cream flex flex-col">
      ...
      <nav className="hidden md:flex items-center justify-between px-8 py-4 bg-white border-b-4 border-navy no-print">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={toggle}
            className="ml-2 px-5 py-3 border-4 border-navy text-navy font-black text-base uppercase tracking-wider hover:bg-navy hover:text-white transition-colors min-h-12 text-size-toggle"
      ...
      <footer className="bg-white border-t-4 border-navy py-8 px-4 text-center no-print">
        ...
        <div className="mt-4 flex items-center justify-center gap-6 text-sm font-black uppercase tracking-wider">
        ...
        <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t-4 border-navy flex items-center justify-around py-2 z-50 bottom-nav no-print">
          {navItems.map(item => (
            <NavLink
              ...
              to={item.to}
              className={({ isActive }) =>
                `flex flex-col items-center gap-1 px-4 py-2 text-xs font-black uppercase tracking-wider transition-colors min-h-12 min-w-[64px] ${
